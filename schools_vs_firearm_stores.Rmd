...
We conduct the main analysis (trimming GPS at 5th/95th percentiles and trimming exposures at the 95th percentile) for one state at a time. For now, we show the results for California, New York, and Texas.

```{r match-states}
# Note: In data_analysis data.frame, state_fips are saved as doubles, not characters, so may be one digit instead of two digits with leading zero

# California
CA_matched_pop <- generate_state_pseudo_pop(6)
make_state_correlation_plot(6, CA_matched_pop)
get_matched_logistic_results(CA_matched_pop)
# get_matched_semiparametric_results(CA_matched_pop)
get_matched_nonparametric_results(CA_matched_pop)

# New York
NY_matched_pop <- generate_state_pseudo_pop(36)
make_state_correlation_plot(36, NY_matched_pop)
get_matched_logistic_results(NY_matched_pop)
# get_matched_semiparametric_results(NY_matched_pop)
get_matched_nonparametric_results(NY_matched_pop)

# Texas
TX_matched_pop <- generate_state_pseudo_pop(48)
make_state_correlation_plot(48, TX_matched_pop)
get_matched_logistic_results(TX_matched_pop)
# get_matched_semiparametric_results(TX_matched_pop)
get_matched_nonparametric_results(TX_matched_pop)



# To Do: for all states; maybe make map; print state name on plots

# all_state_fips <- unique(data_analysis$state_fips)
# match_pop <- lapply(all_states, generate_state_pseudo_pop) # to do: check if all_states must be a list
```
