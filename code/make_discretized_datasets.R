...
confounders_names <- c("census_division_number", paste0(quantitative_confounders))
...
get_undiscretized_dataset <- function(exposure_name){
  cleaned_data <-  fread("data/all_tracts_2020_subset_vars.csv")
  if ("GEOID" %in% colnames(cleaned_data)){
    data_analysis <- get_analysis_df(cleaned_data, exposure_name, c(all_confounder_names, "GEOID"))
  } else{
    data_analysis <- get_analysis_df(cleaned_data, exposure_name, all_confounder_names)
  }
  data_analysis <- na.omit(data_analysis) # omit 784 Census tracts with no mental health index
