...
# Get dataset(s)
data_binary_exposure1mi <- get_binary_exposure_continuous_confounders_dataset("mean_total_miles", F, 1, confounder_names = c(all_confounder_names, "State_Name"))
data_binary_exposure2mi <- get_binary_exposure_continuous_confounders_dataset("mean_total_miles", F, 2, confounder_names = c(all_confounder_names, "State_Name"))

# Get dataset(s) for sensitivity analyses
data_1mi_incl_urban_rural <- get_binary_exposure_continuous_confounders_dataset("mean_total_miles", cutoff = 1, confounder_names = c(all_confounder_names, "State_Name", "urban_rural"))
data_2mi_incl_urban_rural <- get_binary_exposure_continuous_confounders_dataset("mean_total_miles", cutoff = 2, confounder_names = c(all_confounder_names, "State_Name", "urban_rural"))

# Initialize Formulas
formula_matching <- as.formula(paste("a ~", paste(c(all_confounder_names, "State_Name"), collapse = "+", sep = "")))
formula_matching_incl_urban_rural <- as.formula(paste("a ~", paste(c(all_confounder_names, "State_Name", "urban_rural"), collapse = "+", sep = "")))

# Initialize data frames tracking results from all analyses and Std Error calculations
HC1_results <- data.frame(matrix(nrow = 2, ncol = 6)) # Heteroskedasticity Robust SE (MacKinnon and White, 1985)
vcovHC_results <- data.frame(matrix(nrow = 2, ncol = 6)) # Heteroskedasticity Consistent SE (Zeileis, 2006)
vcovCL_results <- data.frame(matrix(nrow = 2, ncol = 6)) # Clustered SE
