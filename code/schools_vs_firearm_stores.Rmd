...
Several confounders are included as a proportion (for instance, of 25+ individuals who have a bachelor's degree) over that Census tract's population (`total_population`; year unknown), which is also a variable in this dataset.

Several potential confounders (for instance, population of 18+ individuals) are not included in this analysis because they have correlation $>0.83$ with a confounder that is included. The only exception is `pop_below18`, which has a correlation coefficient of 0.871 with `total_population`.
...
- `count_gun_dealers` and `count_schools` are included as integers, i.e., not divided by population or area.
...
- `prop_multiracial` is the proportion of individuals in that Census tract who identify as 2+ races; these multiracial individuals' specific races are not included in this analysis. (Would it be logical or strange to try to add a fraction of a person to each of their constituent races?)
- `Shape_Area` and `Shape_Length` are variables in this dataset. Do they measure the area (in what units?) and length (at longest point? in what units?) of a Census tract?
...
- So far, the only region we have excluded is Puerto Rico. Should we also exclude D.C., Alaska, and/or Hawaii? Any other grounds for exclusion?
- Do we have better urban-rural classification than 2013 NCHS? (Michelle found this for free on the NCHS website; it is at the county-level, not Census-tract level)
- Double-check our understanding of `mean_total_miles`. Should we run a sensitivity analysis using `max_total_miles` and/or `min_total_miles` as the exposure?
- We had questions about certain variables; see Data Wrangling section above.
- Any feedback on our variable selection, including our calculation of proportions, is welcome! Any variables we should add besides those listed in Data Wrangling (for instance, home ownership, which may be at the ZIP-code level)?
- Should we try matching within states? When originally discussed, this idea aimed to control for factors such as local gun control laws. However, from a cursory glance at a few states, matching within a state seems to worsen covariate balance.
- What do we think of our parametric vs. non-parametric models and results? Too smooth, smooth enough, not smooth enough? Same question for not trimming vs. trimming at the 95th (and 5th?) percentile vs. trimming at the 99th (and 1st?) percentile.


# Next Steps

- Incorporate feedback, refine the analysis, use more models
- Explore other outcomes: weapon type, preplanned, targets (i.e., random vs targeted vs both vs neither), school level (i.e., elementary, middle, high, etc.), number of shots fired
