...
uncapped_weighting <- function(data, exposure_name){
  ...
  fwrite(results_weight$cov_bal.uncapped, paste0(dir, "results/causal_analyses/uncapped_weighting/", var_arg_a_p_weight, "_correlation.csv"))
  ggsave(paste0(dir, "results/causal_analyses/uncapped_weighting/", var_arg_a_p_weight, "_correlation_plot.png"),
         make_correlation_plot(results_weight$cov_bal.uncapped))
  
  # get mean and max AC of weighted unadjusted pseudopopulation
  results_weight$mean_AC_weighted <- mean(
    results_weight$cov_bal.uncapped[Dataset == "Weighted", `Absolute Correlation`]
  )
  results_weight$mean_AC_unadjusted <- mean(
    results_weight$cov_bal.uncapped[Dataset == "Unadjusted", `Absolute Correlation`]
  )
  ...
  results_weight[["cov_bal.uncapped"]] <- NULL
  
  # save results as txt file
  cat(exposure_name, "weight", "state.urbanity", "5.95",
      sep = "\n",
      file=paste0(dir, "results/causal_analyses/uncapped_weighting/", var_arg_a_p_weight, ".txt"), 
      append=TRUE)
...
uncapped_weighting(any_ffl, exposure_name = "mean_distance_all_persistent_dealers")
uncapped_weighting(commercial_ffl, exposure_name = "mean_dist_commercial_dealers")
